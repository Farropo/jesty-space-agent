...
LM Studio action endpoints expose fixed local operations only: start the default `lms` server on port 1234 and load a selected model through the native localhost REST endpoint. They do not accept arbitrary command args or remote URLs.
...
- LM Studio start and model-load actions stay fixed to localhost port 1234

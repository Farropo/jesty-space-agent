...
- login uses the shared auth service challenge and proof flow
- successful login sets the `space_session` cookie through the auth service
- `guest_create` creates an `L2` guest user and refreshes the watchdog
...
- `user_self_info` returns the authenticated user's derived identity snapshot from the watched indexes
